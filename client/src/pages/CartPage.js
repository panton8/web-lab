...
const CartPage = () => {
  ...
  const getToken = async () => {
    try {
      const { data } = await axios.get("/api/v1/film/braintree/token");
  ...
  const handlePayment = async () => {
    try {
      ...
      const { data } = await axios.post("/api/v1/film/braintree/payment", {
        nonce,
        cart,
      });
      setLoading(false);
            ...
            {cart?.map((f) => (
              <div className="row mb-2 p-3 card flex-row" key={f._id}>
                ...
                <div className="col-md-8">
                  ...
                  <p>Ticket price : {f.ticketPrice}</p>
                  <button
